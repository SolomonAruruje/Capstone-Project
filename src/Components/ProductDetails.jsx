...
const ProductDetails = ({ product }) => {
  ...
  const {
    ...
    discountPercentage = '',
    discountColorCode = '',
    quantity = 0, // This is the max available quantity from backend
    averageRating = 0,
    totalReviews = 0,
    attributes = {},
  } = product;
  ...
  const handleQtyChange = (delta) => {
    setQty(prev => {
      ...
      if (newQty > productStock) return productStock;
      return newQty;
    });
  };
        ...
        <div className="flex flex-col w-full max-w-[400px]">
          ...
          <div className="flex items-center my-2">
            ...
            <p className="ml-2 text-[14px] text-[#000000]">
              ...
              {productStock > 0 ? (
                <span className='text-green-600 font-bold ml-1'>{productStock} in stock</span>
              ) : (
          ...
          <p className="text-[24px] font-semibold">&#8358;{parseFloat(price).toLocaleString()}
            {oldPrice && <span className="ml-4 line-through text-gray-500 text-[20px]">{parseFloat(oldPrice).toLocaleString()}</span>}
          </p>
          ...
          <div className="flex items-center space-x-3 mt-5">
            <div className="flex">
              ...
              <button onClick={() => handleQtyChange(1)} className="w-[30px] border rounded-r" disabled={qty >= productStock}>+</button>
            </div>
            <button onClick={handleAddToCart} className="bg-[#DB4444] text-white px-5 py-2 rounded hover:bg-red-600 transition" disabled={productStock <= 0}>
              Add to Cart
            </button>
