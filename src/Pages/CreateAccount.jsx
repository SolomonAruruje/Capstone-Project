...
const CreateAccount = () => {
    ...
    const { login, loading: authLoading, API_BASE_URL } = useAuth(); // Get API_BASE_URL from AuthContext
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Optionally auto-login after registration if your backend returns token
            if (result.accessToken && result.user) {
                await login(result.accessToken, result.user);
                setTimeout(() => navigate('/checkout'), 1500); // Redirect to checkout if auto-logged in
            } else {
                setTimeout(() => navigate('/login'), 2000); // Else, redirect to login page
            }
            
        } catch (err) {
            console.error('Registration error:', err);
            const backendMessage = err.response?.data?.message || err.message || 'Registration failed. Please try again.';
            ...
            <div className='flex items-center justify-start my-20'>
                ...
                <div className='flex flex-col w-[370px] space-y-6'>
                    ...
                    <div className="text-center mt-4">
                        <p className="text-gray-600">Already have an account? <Link to="/login" className="text-[#DB4444] hover:underline">Log in</Link></p>
                    </div>
                </div>
            </div>
            <Footer />
